...
def lower_intrinsic(
    ...
    if func_name in ("llvm.usub.sat.i8", "llvm.usub.sat.i16", "llvm.usub.sat.i32"):
        return lower_usub_sat(call, fresh)

    if func_name in ("llvm.uadd.sat.i8", "llvm.uadd.sat.i16", "llvm.uadd.sat.i32"):
        return lower_uadd_sat(call, fresh)

    # No-op intrinsics: drop entirely (no codegen effect)
    if func_name == "llvm.assume":
        return []
...
def _get_type_from_suffix(func_name: str) -> JCType:
    """Get JCType from intrinsic name suffix (.i8/.i16/.i32)."""
    if func_name.endswith(".i8"):
        return JCType.BYTE
    if func_name.endswith(".i16"):
        return JCType.SHORT
    if func_name.endswith(".i32"):
        return JCType.INT
    raise ValueError(f"Unknown type suffix: {func_name}")


def lower_usub_sat(call: CallInst, fresh: FreshNameGenerator) -> list[Instruction]:
    """llvm.usub.sat.iN(a, b) -> max(a - b, 0) unsigned.

    Lowered to: cmp = icmp uge a, b; diff = sub a, b; select cmp, diff, 0
    """
    if call.result is None:
        raise ValueError("llvm.usub.sat must have a result")

    ty = _get_type_from_suffix(call.func_name)
    a, b = call.args[0], call.args[1]
    cmp_name = fresh.fresh()
    diff_name = fresh.fresh()
    zero = Const(value=0, ty=ty)

    return [
        ICmpInst(result=cmp_name, pred="uge", left=a, right=b, ty=ty),
        BinaryInst(result=diff_name, op="sub", left=a, right=b, ty=ty),
        SelectInst(
            result=call.result,
            cond=SSARef(cmp_name),
            true_val=SSARef(diff_name),
            false_val=zero,
            ty=ty,
        ),
    ]
...
def lower_uadd_sat(call: CallInst, fresh: FreshNameGenerator) -> list[Instruction]:
    """llvm.uadd.sat.iN(a, b) -> min(a + b, MAX) unsigned.

    Lowered to: sum = add a, b; ovf = icmp ult sum, a; select ovf, MAX, sum
    """
    if call.result is None:
        raise ValueError("llvm.uadd.sat must have a result")

    ty = _get_type_from_suffix(call.func_name)
    a, b = call.args[0], call.args[1]
    sum_name = fresh.fresh()
    ovf_name = fresh.fresh()
    max_val = {JCType.BYTE: 0xFF, JCType.SHORT: 0x7FFF, JCType.INT: -1}[ty]
    max_const = Const(value=max_val, ty=ty)

    return [
        BinaryInst(result=sum_name, op="add", left=a, right=b, ty=ty),
        ICmpInst(result=ovf_name, pred="ult", left=SSARef(sum_name), right=a, ty=ty),
        SelectInst(
            result=call.result,
            cond=SSARef(ovf_name),
            true_val=max_const,
            false_val=SSARef(sum_name),
            ty=ty,
        ),
    ]
