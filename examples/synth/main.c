...
void process(APDU apdu, short apdu_len) {
    ...
    if (ins == INS_GENERATE) {
        ...
        APDU_setOutgoingLength(apdu, AUDIO_BUFFER_SIZE);
        APDU_sendBytesLong(apdu, audio_buffer, 0, AUDIO_BUFFER_SIZE);
        return;
    }
    ...
    if (ins == INS_GENERATE_FAST) {
        ...
        APDU_setOutgoing(apdu);
        APDU_setOutgoingLength(apdu, FAST_BUFFER_SIZE);
        APDU_sendBytesLong(apdu, audio_buffer, 0, FAST_BUFFER_SIZE);
        return;
    }
