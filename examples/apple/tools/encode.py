...
SCREEN_W = 32
SCREEN_H = 20
FB_SIZE = SCREEN_W * SCREEN_H // 8  # 80 bytes
...
def frame_to_bytes(frame: np.ndarray) -> bytes:
    ...
    for y in range(SCREEN_H):
        for byte_idx in range(4):  # 4 bytes per row (32 pixels / 8)
            byte_val = 0
            for bit in range(8):
                x = byte_idx * 8 + bit
                if frame[y, x] > 0:
                    byte_val |= 0x80 >> bit
            result[y * 4 + byte_idx] = byte_val
    return bytes(result)
...
def generate_header(frames: list[bytes], output_path: str, source_name: str, fps: float, use_delta: bool = True):
    # Compress all frames (with optional delta encoding)
    ...
    if total_size > 60000:
        print(f"\nWARNING: Compressed size ({total_size:,}) may exceed JavaCard flash!")
        print("Consider reducing FPS or using a shorter clip.")

    with open(output_path, "w") as f:
        f.write(f"// video.h - Bad Apple compressed frames\n")
        f.write(f"// Generated by encode.py\n")
        f.write(f"// Source: {source_name} @ {fps} fps, {SCREEN_W}x{SCREEN_H} @ 1bpp\n")
...
def preview_frames(frames: list[np.ndarray], count: int = 5):
    for i, frame in enumerate(frames[:count]):
        print(f"\n--- Frame {i} ---")
        for y in range(SCREEN_H):
            print("".join(" #"[1 if frame[y, x] > 0 else 0] for x in range(SCREEN_W)))


def main():
    parser = argparse.ArgumentParser(description="Convert video to Bad Apple C header")
    parser.add_argument("video", help="Input video file")
    parser.add_argument("-o", "--output", default="video.h", help="Output header file")
    parser.add_argument("--fps", type=float, default=5, help="Target FPS (default: 5)")
    ...
    print("Converting frames to bytes...")
    frame_bytes = [frame_to_bytes(f) for f in frames]

    generate_header(frame_bytes, args.output, Path(args.video).name, args.fps)
